...
class ECCharbonnierLoss(nn.Module):
    ...
    def forward(self, pred_cloudfree, cloudfree, cloudmask):
        cloudmask = cloudmask.unsqueeze(
            1).expand(-1, pred_cloudfree.shape[1], -1, -1)
        weight = torch.ones_like(cloudmask) + self.EC_weight*cloudmask
        loss = torch.mean(weight*torch.pow(((pred_cloudfree-cloudfree)
                          ** 2 + self._epsilon ** 2), self._alpha))
        return loss
